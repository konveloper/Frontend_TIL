...
## 자바스크립트 삽입위치
* head, body의 문서 처음, 중간, 끝 // 미리 로드가 되어야하는 것들은 스크립트를 처음에 배치
* 보통 body의 맨 끝
```js
<!DOCTYPE html>
<html lang="en">
...
// 주의할 점 : 모든 a.js b.js가 합친 것처럼 동작. 변수 이름이 중복되었을 때 에러가 난다.
```
...
// alert('hello')
// window.alert('hello')
// confirm.('hello') 확인 취소창이 뜬다
4. 콘솔에 찍는 방법(console.log, console.table, console.error 등)

## 코드 구조
1. 문(statement)은 세미콜론으로 구분 //세미콜론을 붙이지 않는 회사 컨벤션도 있습니다.
2. 문은 값, 연산자, 키워드, 명령어, 표현식(값으로 평가, 함수나 key, index를 통한 값의 호출도 표현식) 등으로 구성됩니다.
3. 공백 병합
    // 괄호 사이 공백 병합함. 난독화 : 모든 띄어쓰기 등을 없애기 + 컴프레스도 함께 => 코드도 못 가져가고 용량도 줄이게
```js
let x = 10, 
    y = 20, 
    z = 30
...
* 관계 연산자
    * 키만 가지고 판단
    ```js
    10 in [10, 20, 30] // false (키값이 없어서)
    1 in [10, 20, 30] // true (20이 1을 보유하고 있어서)
    1 in 'hello' // error
...
// function 예전엔 typeof 찍으면 object로 나왔음 지금은 function 나옴 근데 object처럼 쓸 수 있어서 
...
- Number(숫자)
    ...
    * Math
        ...
        * Math.random()
        // 0.126... 
        // *11 하면 0~10 사이 (원하는 숫자에서 하나 많게 곱한다)
        // Math.floor(Math.random()*11) // 0~10 숫자가 나온다
        * Math.abs()
        * Math.sqrt()
        * Math.pow()
    * NaN
...
- String(문자열)
    ...
    * 메서드 : 
        * str.length
        * str.indexOf()
        * str.lastIndexOf() // 마지막에 포함하는가 (사용할 일 없다)
        * str.includes()
        * str.slice()
        * str.split()
        * str.substring()
        ...
        * str.repeat()
        ...
        'hello'.repeat(100)
        '0'.repeat(100)
        '5'.repeat(100).split('').map(Number) // <- 콜백함수가 Number로 들어간 것
        ```
    ...
    * 메서드 : 
        ...
        * map (반환값으로 새로운 어레이)
        ...
        // [undefined, undefined, undefined]
            리턴된 값으로 새로운 어레이를 만드는데 undefined
        * filter
        // test.filter(e => e >= 200) // [200, 300]
        // 비교) test.map(e => e >= 200) // [false, true, true]
        * push / pop - mutable (수정 가능한) - 원본 수정
        * slice - immutable (수정이 불가능한) - 원본 수정되지 않음
        * splice - mutable
        * reduce - immutable 
        * join
        * indexOf
...
- do while
- forEach
- break // 한꺼풀만 탈출, 코드 블록 탈출, 가까운 반복문 탈출
- continue // 그 다음 블럭으로 넘어감
...
- 함수 표현식과 함수 선언식
    ```js
    let 함수표현식 = function(){} // 호이스팅 O (단 tdz에 들어가서 안 되는 것처럼 보인다)
...
- 콜백함수
    ...
    * 화살표 함수를 콜백함수로 사용했을 경우의 장단점 (재사용하지 않을 간단한 코드만 화살표 함수로!)
        * 장점 : 네이밍을 안해도 됩니다.
        * 단점 : 콜백지옥에 빠질 수가 있습니다.
                 다른 곳에서 사용할 수가 없다.
    ```js
...
* 클래스 - 붕어빵 찍는 틀, 공장
* 인스턴스 - 붕어빵, 제품
// 클래스가 생성자함수나 클로저를 대체할 수 있다. 반대는 안 된다.
...
// 틀만 만들어놓고 인스턴스에 값을 입력
게시물하나 = new 게시물("오늘도 JS에게 조져졌다", "곤")];
게시물하나 = new 게시물("오늘도 JS에게 조져졌다", "곤");
```
...
## 사용자와 상호작용
* prompt('hello') // prompt로 얻은 값은 스트링
* comfirm('hello')
* alert('hello')
